...
import { MapPin, ArrowLeft, Building2, Loader2 } from 'lucide-react';
import { getCompaniesByCity, getCompaniesWithMinPrice } from '../services/companySearchService';
...
export const CidadePage = () => {
  ...
  useEffect(() => {
    if (!city) return;
    setLoading(true);
    getCompaniesByCity(cityName).then(async (data) => {
      ...
      <div className="py-12 px-4 sm:px-6" style={{ background: 'linear-gradient(160deg, #040c1a 0%, #071628 100%)' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-slate-400">
            Encontre lavagem, polimento, higienização e serviços automotivos perto de você.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        {loading ? (
          <div className="flex items-center justify-center py-24 gap-3 text-slate-400">
            ...
            <span>Carregando estéticas em {cityName}...</span>
          </div>
        ) : companies.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            <Building2 className="w-14 h-14 text-slate-700" />
            <div>
              <p className="text-white font-black text-lg mb-1">Nenhuma estética em {cityName}</p>
              <p className="text-slate-500 text-sm">Seja o primeiro a cadastrar sua estética nessa cidade!</p>
            </div>
          ...
          <>
            <p className="text-sm text-slate-500 mb-6">
              {companies.length} {companies.length === 1 ? 'estética encontrada' : 'estéticas encontradas'} em {cityName}
            </p>
